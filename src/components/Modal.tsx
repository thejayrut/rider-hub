...
export function ModalHead({kicker,title,onClose,desc}:{kicker:string;title:string;onClose:()=>void;desc?:string}){const close=()=>{if(history.state?.rhModal)history.back();else onClose()};return <div className="modalHead"><div><div className="kicker">{kicker}</div><h3>{title}</h3>{desc&&<p>{desc}</p>}</div><button className="round" onClick={close}>×</button></div>}
