/* Rider Hub deterministic onboarding/auth coordinator.
   Keeps exactly three manual welcome slides stable while account services initialize,
   then enforces motorcycle setup and the optional owner's-manual step for new users. */
(()=>{
'use strict';
...
const SLIDES=[
  ...
  {k:'RIDER HUB',h:'Your bike and rides can follow you across devices.',p:'Sign in to keep your Rider Hub available across your devices while the core experience remains useful even when your connection is limited.',f:[['Private account','Your workspace belongs to your account.'],['Your motorcycle','Bike details, maintenance and documents stay tied to the correct Rider Hub account.']]}
];
let index=0;
let mode='welcome';
...
function renderWelcome(i=index){
  ...
  st.innerHTML=`<div class="rh-slide-kicker">${esc(s.k)}</div><h1>${esc(s.h)}</h1><p>${esc(s.p)}</p><div class="rh-auth-features">${s.f.map(x=>`<div class="rh-auth-feature"><b>${esc(x[0])}</b><span>${esc(x[1])}</span></div>`).join('')}</div>${s.highlight?`<div class="rh-auth-highlight"><strong>${esc(s.highlight[0])}</strong><span>${esc(s.highlight[1])}</span></div>`:''}<div class="rh-auth-dots">${SLIDES.map((_,n)=>`<i class="rh-auth-dot ${n===index?'active':''}"></i>`).join('')}</div><div class="rh-auth-actions">${index?'<button type="button" onclick="rhApprovedWelcomeBack()">Back</button>':''}<button type="button" class="primary" onclick="rhApprovedWelcomeNext()">${index===SLIDES.length-1?'Continue':'Next'}</button></div>`;
}
...
function enforceSignedInGate(){
  ...
  if(window.riderHubManualSetupPending||manualNeeded()){
    mode='manual';
    if(typeof window.riderHubShowManualSetupStep==='function')window.riderHubShowManualSetupStep();
    return;
  }
