...
onAuthStateChanged(auth,user=>{
  if(user){
    ...
    signOut(auth).catch(()=>{}).finally(()=>{current=null;authResolved=true;window.riderHubAuthBlocked?.(String(user.email||'').toLowerCase());window.riderHubAuthReady?.(null)});
    return;
  }
  current=null;window.riderHubFirebaseUser=()=>null;authResolved=true;window.riderHubSignedOut?.();window.riderHubAuthReady?.(null);
});
