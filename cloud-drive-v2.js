/* Rider Hub private-file backup in the signed-in user's own Google Drive.
   The user's Drive connection preference persists until they explicitly disconnect.
   Short-lived Google access tokens are refreshed only when a Drive action needs them. */
(()=>{
'use strict';
...
function remembered(){
  const c=readCfg(),user=firebaseUser(),saved=String(c.email||'').toLowerCase(),active=String(user?.email||'').toLowerCase();
  return !!saved&&c.desired!==false&&(!active||saved===active);
}
function driveStatus(){const c=readCfg();return{connected:connected(),remembered:remembered(),email:c.email||'',lastSync:c.lastSync||'',desired:c.desired!==false&&!!c.email}}
...
async function api(url,opt={}){
  if(!connected())throw new Error('Google Drive access needs to be refreshed');
  const headers=new Headers(opt.headers||{});headers.set('Authorization','Bearer '+accessToken);
  const r=await fetch(url,{...opt,headers});
  if(r.status===401){accessToken='';tokenExpiry=0;writeCfg({lastAccessExpiredAt:new Date().toISOString()});throw new Error('Google Drive access needs to be refreshed')}
  if(!r.ok){const text=await r.text().catch(()=>'');let msg=text;try{msg=JSON.parse(text)?.error?.message||text}catch{}throw new Error(msg||`Google Drive error ${r.status}`)}
  return r;
}
...
async function requestDriveAccess(syncAfter=false,forceConsent=false){
  ...
  try{
    ...
    const auth=authMod.getAuth(app),provider=new authMod.GoogleAuthProvider();
    provider.addScope('https://www.googleapis.com/auth/drive.file');provider.addScope('email');provider.addScope('profile');
    provider.setCustomParameters({prompt:forceConsent||!remembered()?'consent':'',login_hint:user.email||''});
    const result=await authMod.reauthenticateWithPopup(auth.currentUser,provider),credential=authMod.GoogleAuthProvider.credentialFromResult(result);
    if(!credential?.accessToken)throw new Error('Google Drive permission was not returned');
    accessToken=credential.accessToken;tokenExpiry=Date.now()+55*60*1000;
    const profile=await googleProfile(),expected=String(user.email||'').toLowerCase(),actual=String(profile.email||'').toLowerCase();
    if(expected&&actual&&expected!==actual){accessToken='';tokenExpiry=0;throw new Error('Connect the same Google account you use for Rider Hub')}
    const prev=readCfg();if(prev.email&&actual&&String(prev.email).toLowerCase()!==actual)writeCfg({rootId:'',docsId:'',manualsId:''});
    const folders=await ensureFolders();
    writeCfg({desired:true,email:profile.email||user.email||'',name:profile.name||user.displayName||'',connectedAt:prev.connectedAt||new Date().toISOString(),lastAccessAt:new Date().toISOString(),...folders});
    const filesOk=await backupAllLocalFiles();refreshUi();
    window.toast?.(filesOk?'Google Drive connected':'Google Drive connected · some files are pending');
    if(syncAfter&&typeof window.renderMore==='function')window.renderMore();
    return true;
  }catch(e){
    console.warn('Rider Hub Drive connection failed',e);const code=String(e?.code||'');
    if(code.includes('popup-closed')||code.includes('cancelled'))window.toast?.('Google Drive connection cancelled');
    else if(remembered())window.toast?.('Google Drive needs permission again');
...
function disconnect(){accessToken='';tokenExpiry=0;writeCfg({desired:false,disconnectedAt:new Date().toISOString()});refreshUi();window.toast?.('Google Drive disconnected')}
